...
export const PlacesApi = {
  ...
  create: async (
    place: Omit<SlagFieldPlace, 'id'>
  ): Promise<SlagFieldPlace> => {
...
export const SlagFieldApi = {
  ...
  getStateSnapshot: async (timestamp: Date): Promise<RawHistoryEvent[]> => {
    return axiosApi<RawHistoryEvent[]>(
      `/slagfield/state/snapshot?timestamp=${timestamp.toISOString()}`
    );
  },
  ...
  placeBucket: async (
    ...
    await axiosApi<any>(
      `/slagfield/places/${placeId}/place-bucket`,
      'POST',
      data
    );
    return true;
  ...
  emptyBucket: async (
    ...
    try {
      await axiosApi<any>(
        `/slagfield/places/${placeId}/empty-bucket`,
        'POST',
        data
      );
      return true;
    } catch (err: any) {
      // Если backend вернул 400 — это не «сеть отвалились», а именно бизнес‑ошибка
      if (err.response?.status === 400) {
        return false;
      }
      // Все остальные — пробрасываем наверх, чтобы handleEmptyBucket ушёл в catch
      throw err;
    }
  },
