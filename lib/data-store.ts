...
class DataStore {
  ...
  addPlace(place: Omit<SlagFieldPlace, 'id'>): SlagFieldPlace {
    ...
    this.addToHistory(
      ...
      newPlace.id,
      `Добавлено место: ${
        place.name || `${place.row || 0}-${place.number || 0}`
      }`
    );
  ...
  updatePlace(
    id: string,
    place: Partial<SlagFieldPlace>
  ): SlagFieldPlace | undefined {
  ...
  deletePlace(id: string): boolean {
    ...
    if (deleted && placeToDelete) {
      ...
      this.addToHistory(
        ...
        `Удалено место: ${
          placeToDelete.name ||
          `${placeToDelete.row || 0}-${placeToDelete.number || 0}`
        }`
      );
  ...
  updateBucket(id: string, bucket: Partial<Bucket>): Bucket | undefined {
    ...
    if (index !== -1) {
      ...
      this.addToHistory(
        ...
        'bucket',
        id,
        `Обновлен ковш: ${bucket.name || this.buckets[index].name}`
      );
  ...
  deleteBucket(id: string): boolean {
    ...
    if (deleted && bucketToDelete) {
      ...
      this.addToHistory(
        ...
        'bucket',
        id,
        `Удален ковш: ${bucketToDelete.name}`
      );
  ...
  addMaterial(material: Omit<Material, 'id'>): Material {
    ...
    this.addToHistory(
      ...
      'material',
      newMaterial.id,
      `Добавлен материал: ${material.name}`
    );
  ...
  updateMaterial(
    ...
    if (index !== -1) {
      ...
      this.addToHistory(
        ...
        'material',
        id,
        `Обновлен материал: ${material.name || this.materials[index].name}`
      );
  ...
  deleteMaterial(id: string): boolean {
    ...
    if (deleted && materialToDelete) {
      ...
      this.addToHistory(
        ...
        'material',
        id,
        `Удален материал: ${materialToDelete.name}`
      );
  ...
  getEnrichedSlagFieldStates() {
    return this.slagFieldStates.map((state) => {
      ...
      const bucket = state.bucketId
        ? this.getBucket(state.bucketId)
        : undefined;
      const material = state.materialId
        ? this.getMaterial(state.materialId)
        : undefined;
  ...
  placeBucket(data: {
    ...
    this.addToHistory(
      ...
      'slagField',
      newState.id,
      `Ковш ${bucket.name} размещен на месте ${
        place.name || `${place.row || 0}-${place.number || 0}`
      }`
    );
  ...
  emptyBucket(data: {
    ...
    if (place && bucket && material) {
      ...
      const historyRecord: HistoryRecord = {
        ...
        placeId: placeId,
        placeRow:
          typeof place.row === 'string'
            ? Number.parseInt(place.row)
            : place.row || 0,
  ...
  removeBucket(placeId: string): boolean {
    ...
    const stateIndex = this.slagFieldStates.findIndex(
      (state) => state.placeId === placeId && state.status === 'active'
    );
    ...
    if (removedState) {
      ...
      if (place && bucket && material) {
        this.addToHistory(
          ...
          'slagField',
          removedState.id,
          `Ковш ${bucket.name} удален с места ${
            place.name || `${place.row || 0}-${place.number || 0}`
          }`
        );
      }
  ...
  addMaterialSetting(setting: Omit<MaterialSetting, 'id'>): MaterialSetting {
    ...
    this.addToHistory(
      ...
      'materialSetting',
      newSetting.id,
      `Добавлена настройка: ${setting.name}`
    );
  ...
  updateMaterialSetting(
    id: string,
    setting: Partial<MaterialSetting>
  ): MaterialSetting | undefined {
    const index = this.materialSettings.findIndex((s) => s.id === id);
    if (index !== -1) {
      this.materialSettings[index] = {
        ...this.materialSettings[index],
        ...setting,
      };
      ...
      this.addToHistory(
        ...
        id,
        `Обновлена настройка: ${
          setting.name || this.materialSettings[index].name
        }`
      );
  ...
  deleteMaterialSetting(id: string): boolean {
    ...
    this.materialSettings = this.materialSettings.filter(
    ...
    if (deleted && settingToDelete) {
      ...
      this.addToHistory(
        ...
        'materialSetting',
        id,
        `Удалена настройка: ${settingToDelete.name}`
      );
  ...
  private initializeMaterials() {
    this.materials = [
      {
        id: '165b725b-352f-4453-a73e-f08089b333a8',
        name: 'Шлак мартеновский',
        isDelete: false,
      },
      {
        id: '2267c5cd-b1a6-43d6-bbd7-317f2e172553',
        name: 'Шлак стальной',
        isDelete: false,
      },
      {
        id: 'a2c17001-1a28-4e00-8ce0-ffd31a4510b4',
        name: 'Шлак доменный',
        isDelete: false,
      },
    ];
  ...
  private loadFromStorage(): void {
    ...
    try {
      ...
      const materialSettingsJson = localStorage.getItem(
        STORAGE_KEYS.MATERIAL_SETTINGS
      );
  ...
  private saveToStorage(): void {
    ...
    try {
      ...
      localStorage.setItem(
        STORAGE_KEYS.MATERIAL_SETTINGS,
        JSON.stringify(this.materialSettings)
      );

      // Сохранение состояний шлакового поля
      localStorage.setItem(
        STORAGE_KEYS.SLAG_FIELD_STATES,
        JSON.stringify(this.slagFieldStates)
      );
...
export const updateMaterialSetting =
  dataStore.updateMaterialSetting.bind(dataStore);
