...
export default function HistoryPage() {
  ...
  const bucketMap = Object.fromEntries(
    buckets.map((b) => [b.id, b.description])
  );
  ...
  const Detail = ({
    ...
    <div className='flex-1'>
      ...
      <div className='bg-gray-50 px-3 py-1.5 rounded-md overflow-hidden'>
        {children}
      </div>
    </div>
  );
    ...
    <DashboardLayout>
      ...
      <div className='space-y-4'>
        ...
        {dailyEvents.map((evt) => {
          ...
          const isPlaceEvt =
            evt.eventType === 'WentInUse' ||
            evt.eventType === 'WentOutOfUse' ||
            evt.eventType === 'OutOfUse';
                    ...
                    <>
                      ...
                      <div className='grid grid-cols-3 gap-4'>
                        ...
                        {evt.eventType === 'PlaceBucket' && (
                          <>
                            <Detail label='Ковш'>
                              {bucketMap[evt.bucketId!]}
                            </Detail>
                            <Detail label='Материал'>
                              {materialMap[evt.materialId!]}
                            </Detail>
                          </>
                        )}
                        {evt.eventType === 'PlaceBucket' &&
                        ...
                        {evt.eventType === 'PlaceBucket' &&
                          evt.clientStartDate && (
                            <Detail label='Время установки'>
                              {fmtDT(evt.clientStartDate)}
                            </Detail>
                          )}
                        {evt.eventType === 'EmptyBucket' &&
                          evt.bucketEmptiedTime && (
                            <Detail label='Время опустошения'>
                              {fmtDT(evt.bucketEmptiedTime)}
                            </Detail>
                          )}
