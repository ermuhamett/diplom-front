...
export default function PlacesPage() {
  ...
  const openAddDialogWithRowData = (rowNumber: number) => {
    ...
    const rowPlaces = places.filter((place) => place.row === Number(rowNumber))
    ...
    if (rowPlaces.length > 0) {
      ...
      nextPlaceNumber = Math.max(...rowPlaces.map((place) => place.number)) + 1
    }
              ...
              <CardContent className="pt-4 bg-white dark:bg-card">
                <div className="flex flex-wrap gap-4">
                  {rowPlaces.map((place) => (
                    ...
                    >
                      <span className="text-xl font-bold">{place.row * 100 + place.number}</span>
